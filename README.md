...
- **Workspace file browser** — Upload, edit, and manage files in your persistent home directory. Monaco Editor for code, Milkdown for Markdown, image and PDF viewers.
- **Multi-provider LLM support** — Bring your own API keys for Anthropic, OpenAI, or Google. Switch models mid-conversation.
- **Per-user isolation** — Each user gets their own Kubernetes pod with a persistent home directory.
...
```bash
npm install
npm run dev:setup    # create kind cluster (idempotent)
